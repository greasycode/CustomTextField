...
class S3BotoStorage(Storage):
    ...
    def _save(self, name, content):
        ...
        real_filename = content.name
        headers = self.headers.copy()
        filename = name.encode('utf8', 'ignore').split('/')[-1]
        if self.zip_n_save:
            ...
            zf.close()
            content = file_system.open(zipped_file)
            content_type = getattr(content, 'content_type', mimetypes.guess_type(zipped_file)[0] or Key.DefaultContentType)
            

        else:
           file_path = file_system.save(filename, ContentFile(content.read()))
           content = file_system.open(file_path)
           content_type = getattr(content, 'content_type', mimetypes.guess_type(file_path)[0] or Key.DefaultContentType)
        ...
        key.set_metadata('real_filename', real_filename)
        key.set_metadata('filename_in_zip', filename)
        if self.zip_n_save:
            print str(self.zip_n_save)
            key.set_metadata('zipped', str(self.zip_n_save))
    ...
    def __setstate__(self, dict):
        ...
        self.connection = S3Connection(access_key,secret_key)
        #self._bucket = self._get_or_create_bucket(self.bucket_name)
#        setattr(self.attr_class, "storage", self)	
...
class S3BotoStorageFile(File):
    ...
    def real_filename(self):
        return self.key.metadata['real_filename']
    ...
    def read(self, *args, **kwargs):
        ...
        filename = self.key.metadata['filename_in_zip']

        if self.key.metadata.get('zipped', None):

            zipped_filename = filename + ".zip"
            zipped_file = file_system.save(zipped_filename,
                                       ContentFile(self.key.read()))
            ...
            file_path = file_system.save(filename, ContentFile(self.key.read()))
            filecontent = file_system.open(file_path)
